...
class PySharedStack(Stack):
    ...
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        self.my_bucket = s3.Bucket(self, "Py_Shared_Bucket",
                                   bucket_name=f"py-bucket-{suffix}",
                                   lifecycle_rules=[
                                       s3.LifecycleRule(expiration=Duration.days(5))]
                                   )

        CfnOutput(self, "Py_Shared_Bucket: ", value=self.my_bucket.bucket_name)

        self.my_bucket.apply_removal_policy(RemovalPolicy.DESTROY)

        # Dodanie tagu
        cdk.Tags.of(self.my_bucket).add("suffix", suffix)
    ...
    # Dodanie właściwości dzięki której zasób może być widoczny poza stackiem (getter)
    @property
    def my_shared_bucket(self):
